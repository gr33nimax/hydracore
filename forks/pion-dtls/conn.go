...
func (c *Conn) Read(buff []byte) (n int, err error) { //nolint:cyclop
	...
	for {
		...
		case out, ok := <-c.decrypted:
			...
			switch val := out.(type) {
			case ([]byte):
				// This channel has one consumer and delivers each payload once, and the payload
				// is not referenced again after the copy below — nor on the too-small path — so
				// this is where the record buffer goes back to the pool it came from in
				// ApplicationData.Unmarshal.
				if len(buff) < len(val) {
					protocol.ReleaseApplicationData(val)
