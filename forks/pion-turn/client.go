...
func (c *Client) handleChannelData(data []byte) error {
	// Decoded in place, without taking a copy of the datagram.
	//
	// Ownership: `data` is a slice of the single buffer owned by the read loop in Listen, and
	// this call is synchronous, so it stays valid until we return. Decode only reads the
	// four-byte header and reslices Raw into Data — it retains nothing. The one consumer,
	// relayedConn.HandleInbound below, is synchronous too and takes its own copy before the
	// datagram crosses to another goroutine. So the copy made here was paid on every inbound
	// packet and dropped again a few lines later; profiled on Android it was the largest
	// single allocation site in the process.
	chData := proto.ChannelData{Raw: data}
