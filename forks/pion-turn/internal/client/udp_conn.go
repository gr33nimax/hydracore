...
// pooledInboundSize is the buffer a queued datagram is kept in while it waits for a reader.
//
// A relayed datagram is bounded by the path MTU, so this covers every real packet with room to
// spare; anything larger falls back to a plain allocation and is not returned to the pool. The
// alternative — pooling by maxDataBufferSize, which is 65535 — would let a full read queue of
// 1024 entries hold 64 MB.
const pooledInboundSize = 2048

var inboundBufferPool = sync.Pool{
	New: func() any {
		buffer := make([]byte, pooledInboundSize)

		return &buffer
	},
}

// takeInboundBuffer returns a buffer of exactly n bytes, pooled when n allows it.
func takeInboundBuffer(n int) []byte {
	if n > pooledInboundSize {
		return make([]byte, n)
	}

	return (*inboundBufferPool.Get().(*[]byte))[:n]
}

// releaseInboundBuffer returns a buffer taken by takeInboundBuffer. Buffers that were too large
// to come from the pool are left to the collector, which is why the capacity is checked rather
// than the length.
func releaseInboundBuffer(buffer []byte) {
	if cap(buffer) != pooledInboundSize {
		return
	}
	full := buffer[:pooledInboundSize]
...
func (c *UDPConn) ReadFrom(p []byte) (n int, addr net.Addr, err error) {
	for {
		...
		case ibData := <-c.readCh:
			...
			short := n < len(ibData.data)
			// The queue is drained by this method and by nothing else, and the datagram is not
			// referenced again after the copy above — on either path — so this is where the
			// buffer goes back.
			releaseInboundBuffer(ibData.data)
			if short {
...
func (c *UDPConn) HandleInbound(data []byte, from net.Addr) {
	// The copy stays: `data` belongs to the caller's read loop and is overwritten on its next
	// read, while this datagram waits in a queue for another goroutine. What changes is where
	// the memory comes from — a pool rather than a fresh allocation per packet — and it is
	// returned in ReadFrom once the payload has been copied out.
	copied := takeInboundBuffer(len(data))
	copy(copied, data)

	select {
	case c.readCh <- &inboundData{data: copied, from: from}:
	default:
		// Dropped, so nothing will ever read it: give the buffer back here instead.
		releaseInboundBuffer(copied)
