# HydraCore performance checks

`verify_hydracore_performance.sh` is the repeatable core gate used by CI. It:
...
- rejects a run that exceeds the versioned time, memory, or allocation ceilings in `HYDRACORE_PERFORMANCE_BASELINE`.

Run it from the repository root:

```bash
bash release/verify_hydracore_performance.sh
```
...
The sampler writes CSV after every measurement and fails at the end if the release exceeds the Android ceilings in `HYDRACORE_PERFORMANCE_BASELINE`. Only change those ceilings together with a recorded device result and an explanation in the core release notes.
